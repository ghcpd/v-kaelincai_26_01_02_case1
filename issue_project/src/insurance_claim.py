...
class ClaimCalculator:
    ...
    BUG INTENTIONALLY PLANTED:
    - Boundary condition error: When loss_amount equals exactly 2000, 
      the deductible calculation uses wrong tier (10% instead of 500 fixed)
    - Missing driving years bonus calculation
    """
    ...
    def _calculate_deductible(self, loss_amount: float) -> float:
        ...
        BUG: Boundary condition error at exactly 2000
        When loss_amount == 2000, should use 500 fixed deductible,
        but code incorrectly uses the > 2000 tier (10%).
